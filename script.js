...
async function piSignIn() {
  try {
   ...
   try {
  // Firebase anonymous sign-in first
  const userCredential = await auth.signInAnonymously();
  const firebaseUid = userCredential.user.uid; // تأكد أن auth انتهى
  console.log("Firebase UID:", firebaseUid);

  // ثم تعامل مع Firestore
  ...
  currentUser = { uid: firebaseUid, username: piUser.username, ...userDoc.data() };
