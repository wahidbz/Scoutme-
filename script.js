...
const firebaseConfig = {
  ...
  databaseURL: "https://scoutme-73fb8-default-rtdb.europe-west1.firebasedatabase.app",
  projectId: "scoutme-73fb8",
  storageBucket: "scoutme-73fb8.firebasestorage.app",
  messagingSenderId: "394693573994",
  appId: "1:394693573994:web:752696f514bf8f17f13913"
};
...
const PI_NETWORK_SANDBOX = window.location.hostname === 'localhost' || 
...
async function piSignIn() {
  try {
    ...
    const authResult = await Pi.authenticate(scopes, async (payment) => {
      try {
    ...
    try {
      ...
      console.warn('Firebase anonymous sign in failed, check if enabled in console:', fbError);
    }
    ...
    try {
      ...
      if (!userDoc.exists) {
        ...
        userDoc = await userRef.get(); // Refresh doc
      } else {
        // Existing user
      ...
      throw new Error("Firestore permission denied. Please check your Firebase Rules.");
    }
...
/**
 * Helper to make API calls to backend functions
 */
async function apiPost(path, payload) {
  const isNetlify = window.location.hostname.endsWith(".netlify.app") || window.location.hostname.endsWith(".netlify.live") || window.location.hostname.includes("nexpie.com");
  const functionMap = {
      ...
      Pi.createPayment(paymentData, {
        onReadyForServerApproval: async (paymentId) => {
          try {
            ...
            await apiPost("/payment/approve", { paymentId: paymentId });
            resolve(paymentId);
          } catch (err) {
            console.error(err);
            showToast('تعذر موافقة السيرفر على الدفع.', 'error');
            reject(err);
          }
        },
        onReadyForServerCompletion: async (paymentId, txid) => {
          try {
            ...
            console.error(err);
            showToast('فشل إكمال الدفع من السيرفر.', 'error');
          }
        },
        onCancel: (paymentId) => {
          ...
          if (paymentId) {
            apiPost("/payment/cancel", { paymentId: paymentId }).catch(() => {});
          }
...
/**
 * Auto sign in with Pi Network
 */
async function autoSignInOnLoad() {
  try {
    const scopes = ['username'];
    await Pi.authenticate(scopes, async (payment) => {
      try {
        const paymentId = payment && payment.identifier;
        const txid = payment && payment.transaction && payment.transaction.txid;
        if (paymentId && txid) {
    ...
    // If authentication successful, trigger full sign in
    await piSignIn();
  } catch (error) {
    console.log('Auto sign in failed, user needs to click login');
  }
}
